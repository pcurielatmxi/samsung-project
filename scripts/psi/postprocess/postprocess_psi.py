#!/usr/bin/env python3
"""
Postprocess PSI quality inspection JSON files to normalized CSV tables.

Input: {WINDOWS_DATA_DIR}/processed/psi/v2/*.format.json (6,309 files)
Output:
    - psi_inspections.csv - Main inspection records
    - psi_parties.csv - Parties involved (with company matching)
    - psi_issues.csv - Deficiencies/issues found
    - processing_errors.csv - Error log

Usage:
    python postprocess_psi.py [--output-dir /path/to/output]
"""
...
def build_main_table(records: List[Dict]) -> pd.DataFrame:
    ...
    for record in records:
        try:
            ...
            # Extract inspection_id from filename
            ...
            row = {
                ...
                'inspection_type_normalized': normalize_inspection_type(
                    content.get('inspection_type')
                ),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Postprocess PSI quality inspection JSON files to normalized CSV'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
    ...
    # Paths
    if settings:
        input_dir = settings.PSI_PROCESSED_DIR / 'v2'
    else:
        input_dir = Path('/mnt/c/Users/pdcur/OneDrive - MXI/Desktop/Samsung Dashboard/Data/processed/psi/v2')

    if args.output_dir:
        output_dir = Path(args.output_dir)
    elif settings:
        output_dir = settings.PSI_PROCESSED_DIR / 'v3'
    else:
        output_dir = Path('/mnt/c/Users/pdcur/OneDrive - MXI/Desktop/Samsung Dashboard/Data/processed/psi/v3')

    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("PSI Quality Inspection Postprocessing")
    print("=" * 70)
    ...
    inspections_df.to_csv(output_dir / 'psi_inspections.csv', index=False)
    print(f"  ✓ psi_inspections.csv ({len(inspections_df)} rows)")

    if len(parties_df) > 0:
        parties_df.to_csv(output_dir / 'psi_parties.csv', index=False)
        print(f"  ✓ psi_parties.csv ({len(parties_df)} rows)")

    if len(issues_df) > 0:
        issues_df.to_csv(output_dir / 'psi_issues.csv', index=False)
        print(f"  ✓ psi_issues.csv ({len(issues_df)} rows)")
