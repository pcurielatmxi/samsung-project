...
class DimLocation(BaseModel):
    ...
    Records: ~523 location codes (rooms, elevators, stairs, gridlines, etc.)
    ...
    location_code: str = Field(description="Location code (FAB112345, ELV-S, STR-A)")
    location_type: str = Field(description="Type: ROOM, ELEVATOR, STAIR, GRIDLINE, LEVEL, BUILDING, AREA")
    room_name: Optional[float] = Field(default=None, description="Human-readable room name (often empty)")
    building: str = Field(description="Building code: FAB, SUE, SUW, FIZ, SITE")
    level: Optional[str] = Field(default=None, description="Level: 1F, 2F, B1, ROOF, etc.")
    grid_row_min: Optional[float] = Field(default=None, description="Grid row minimum (A-Z, stored as float when empty)")
    grid_row_max: Optional[float] = Field(default=None, description="Grid row maximum (A-Z, stored as float when empty)")
